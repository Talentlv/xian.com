$(function() {
	...
	$('.user-review-imgs img').each(function(i){
		$(this).click(function(){
			var src = $(this).attr('src');
			// $('.user-review-imgs img').removeClass('active');
			$(this).toggleClass('active');
			$($('.user-review-bigimg img')[i]).attr('src',src);
			if ($(this).attr('class')=='') {
				$($('.user-review-bigimg img')[i]).css('display','none');
			}else{
				var count = i;
				$($('.user-review-bigimg img')[i]).css('display','block');
				$($('.user-review-bigimg img')[count-1]).css('display','none');
				$($('.user-review-bigimg img')[count+1]).css('display','none');
			}
		})
	});
	//商品详情页的头部菜单
	$('#yf-head li a').click(function(){
		$(this).parent().addClass("active").siblings().removeClass("active");
	})

	$('#spxq').click(function(){
    	$('body,html').animate({scrollTop:$('.wrap').offset().top},500);
	});
	//点击用户评价的跳转
	$('#yhpj').click(function(){
    	$('body,html').animate({scrollTop:$('.yc-customs-review').offset().top},500);
    })
    //点击购物保障的跳转
    $('#gwbz').click(function(){
    	$('body,html').animate({scrollTop:$('.yf_shopping_protect').offset().top},500);
    })
	...
	function LocalTime(s) {
		...
		$('.cart-addr #addr-date').text(str2+str4);
